...
/// </summary>
public partial class DatEditor : DatEditorCommon
{
    ...
    public override void UpdateShowList(int selection, uint? preserveId = null)
    {
        if (ObjListView != null)
        {
    ...
    private void SprListView_DragSpr(object sender, MouseEventArgs e)
    {
        if (e.LeftButton == MouseButtonState.Pressed)
        {
            ...
            DataObject data = new(SelectedSprites);
    ...
    {
        SelectedSprites = [.. SprListView.SelectedItems.Cast<ShowList>()];
    }
    ...
    protected override void ChangeGroupType(int group)
    {
        var appearance = CurrentObjectAppearance;
        var frameGroup = appearance.FrameGroup;
        var spriteInfo = frameGroup[group].SpriteInfo;

        isObjectLoaded = false;
        A_SprGroups.Value = frameGroup.Count;
        try
        {
            ...
            A_SprAnimation.Value = spriteInfo.Animation != null ? (int)spriteInfo.Animation.SpritePhase.Count : 1;
        }
        catch (Exception)
        {
            MainWindow.Log("Invalid appearance properties for id " + appearance.Id + ", crash prevented.");
        }
        CurrentSprDir = 2;
    ...
    protected override void LoadCurrentObjectAppearances()
    {
        var appearance = CurrentObjectAppearance;
        ...
        {
            flags = new();
            MainWindow.Log("Missing flags for appearance id " + appearance.Id);
        }
        A_FlagId.Value = (int)appearance.Id;
        ...
        A_FlagName.Text = appearance.HasName ? appearance.Name : null;
        A_FlagDescription.Text = appearance.HasDescription ? appearance.Description : null;
        ...
        A_FullInfo.Text = appearance.ToString();
    ...
    protected override void InternalUpdateThingPreview()
    {
        try
        {
            ...
            int imgWidth = (int)Utils.BitmapToBitmapImage(MainWindow.getSpriteStream((int)spriteInfo.SpriteId[0])).Width;
            int imgHeight = (int)Utils.BitmapToBitmapImage(MainWindow.getSpriteStream((int)spriteInfo.SpriteId[0])).Height;

            for (int i = 0; i < gridWidth; i++)
            {
                RowDefinition rowDef = new() {
                    Height = new GridLength(imgHeight)
                };
            ...
            for (int i = 0; i < gridHeight; i++)
            {
                ColumnDefinition colDef = new() {
                    Width = new GridLength(imgWidth)
                };
                SpriteViewerGrid.ColumnDefinitions.Add(colDef);
            }

            var direction = CurrentSprDir;
            if (IsOutfitsMenuOpened())
            {
                if ((bool)SprBlendLayers.IsChecked == false)
                {
                    ...
                    int index = GetSpriteIndex(frameGroup, layer, (int)Math.Min(direction, spriteInfo.PatternWidth - 1), addon, mount, (int)SprFramesSlider.Value);
                ...
                {
                    int baseIndex = GetSpriteIndex(frameGroup, 0, (int)Math.Min(direction, spriteInfo.PatternWidth - 1), 0, 0, (int)SprFramesSlider.Value);
                    ...
                    {
                        int baseLayerIndex = GetSpriteIndex(frameGroup, 1, (int)Math.Min(direction, spriteInfo.PatternWidth - 1), 0, 0, (int)SprFramesSlider.Value);
                    ...
                    using (System.Drawing.Graphics g = System.Drawing.Graphics.FromImage(baseBitmap))
                    {
                        ...
                        if ((bool)SprFullAddons.IsChecked)
                        {
                            ...
                            {
                                int addonIndex = GetSpriteIndex(frameGroup, 0, (int)Math.Min(direction, spriteInfo.PatternWidth - 1), x, 0, (int)SprFramesSlider.Value);
                                ...
                                int addonLayerIndex = GetSpriteIndex(frameGroup, 1, (int)Math.Min(direction, spriteInfo.PatternWidth - 1), x, 0, (int)SprFramesSlider.Value);
    ...
    protected override void Spr_Drop(object sender, DragEventArgs e)
    {
    ...
    private void BoxPerDirection_RowEditEnding(object sender, DataGridRowEditEndingEventArgs e)
    {
        ...
        {
            var frameGroup = CurrentObjectAppearance.FrameGroup;
            frameGroup[(int)SprGroupSlider.Value].SpriteInfo.BoundingBoxPerDirection.Clear();
            foreach (var box in BoundingBoxList)
                frameGroup[(int)SprGroupSlider.Value].SpriteInfo.BoundingBoxPerDirection.Add(new Box() { X = box.X, Y = box.Y, Width = box.Width, Height = box.Height });
        }
    }
    protected override void FixSpritesCount()
    {
    ...
    private void A_Texture_ValueChanged(object sender, RoutedPropertyChangedEventArgs<object> e)
    {
        ...
        SpriteInfo spriteInfo = appearance.FrameGroup[(int)SprGroupSlider.Value].SpriteInfo;
        if (frameworkElement.Name == "A_SprGroups")
        {
            if (A_SprGroups.Value == 1 && appearance.FrameGroup.Count == 2)
            {
                appearance.FrameGroup.RemoveAt(1);
            }
            else if (A_SprGroups.Value == 2 && appearance.FrameGroup.Count == 1)
            {
                FrameGroup newFrameGroup = appearance.FrameGroup[0].Clone();
                newFrameGroup.FixedFrameGroup = FIXED_FRAME_GROUP.OutfitMoving;
                appearance.FrameGroup.Add(newFrameGroup);
            }
        }
    ...
    private void ObjectSave_PreviewMouseLeftButtonDown(object sender, System.Windows.Input.MouseButtonEventArgs e)
    {
        ...
        if (!string.IsNullOrWhiteSpace(A_FlagName.Text))
            appearance.Name = A_FlagName.Text;

        if (!string.IsNullOrWhiteSpace(A_FlagDescription.Text))
            appearance.Description = A_FlagDescription.Text;
        ...
        if (list == null) {
            StatusBar.MessageQueue?.Enqueue($"Failed to save current object - invalid type.", null, null, null, false, true, TimeSpan.FromSeconds(2));
            return;
        }

        // get currently selected appearance
        var oldAppearance = list.FirstOrDefault(a => a.Id == appearance.Id);
        ...
        if (oldAppearance != null)
        {
            ...
            if (oldTransparency != currentTransparency)
                SetObjectTransparency(appearance, (byte)oldTransparency);
        }
        ShowList showList = ObjListView.SelectedItem as ShowList;

        // replace updated appearance
        int index = list.ToList().FindIndex(a => a.Id == showList.Id);
        if (index >= 0)
            list[index] = appearance.Clone();

        // sort after adding the new appearance
        if (showList.Id != appearance.Id) {
            showList.Id = appearance.Id;

            var sorted = list.OrderBy(a => a.Id).ToList();
            list.Clear();
    ...
    {
        File.Copy(Path.Combine(MainWindow._assetsPath, "catalog-content.json"), Path.Combine(MainWindow._assetsPath, "catalog-content.json-bak"), true);
    ...
    public static List<System.Drawing.Bitmap> SplitImage(System.Drawing.Bitmap originalImage)
    {
        ...
        for (int row = 0; row < rows; row++)
        {
            ...
            {
                System.Drawing.Rectangle cloneRect = new(col * 32, row * 32, 32, 32);
    ...
    public static bool IsImageFullyTransparent(System.Drawing.Bitmap image)
    {
    ...
    private async void CompileAsImages(object sender, RoutedEventArgs e)
    {
        ...
        static (int min, int max) GetMinMax(Xceed.Wpf.Toolkit.IntegerUpDown minControl, Xceed.Wpf.Toolkit.IntegerUpDown maxControl, int defaultValue)
        {
    ...
    {
        List<ShowList> selectedItems = [.. SprListView.SelectedItems.Cast<ShowList>()];
    ...
    private void NewObject_PreviewMouseLeftButtonDown(object sender, MouseButtonEventArgs e)
    {
        ...
        if (list == null || things == null) {
            StatusBar.MessageQueue?.Enqueue($"Failed to create object - invalid list type.", null, null, null, false, true, TimeSpan.FromSeconds(2));
            return;
        }

        newObject.Id = (uint)(list.Max(a => a.Id) + 1);
        list.Add(newObject);
    ...
    private void DeleteObject_PreviewMouseLeftButtonDown(object sender, MouseButtonEventArgs e)
    {
        ...
        var things = GetThingsFromDropdown();
        if (list == null || things == null) {
            StatusBar.MessageQueue?.Enqueue($"Failed to delete object - invalid list type.", null, null, null, false, true, TimeSpan.FromSeconds(2));
            return;
        }
        ...
        if (selectedItems.Count != 0) {
            ...
            foreach (var item in selectedItems) {
                Appearance DelObject = new();
                DelObject = list.FirstOrDefault(o => o.Id == item.Id);
    ...
    {
        var animation = CurrentObjectAppearance.FrameGroup[(int)SprGroupSlider.Value].SpriteInfo.Animation;
        if (animation.HasLoopType) {
            var index = SprLoopType.SelectedIndex;
            ...
            if (index  == 0)
                ...
                loopType = ANIMATION_LOOP_TYPE.Counted;
        }
    }
    ...
    private void A_FlagIdCheck_PreviewMouseLeftButtonDown(object sender, MouseButtonEventArgs e)
    {
        ...
        var list = GetThingsFromDropdown();
        if (list == null) {
            StatusBar.MessageQueue?.Enqueue($"Failed to check object - invalid list type.", null, null, null, false, true, TimeSpan.FromSeconds(2));
            return;
        }

        uint minId = (uint)(ObjectMenu.SelectedIndex == 1 ? 100 : 0);

        if (!list.Any(a => a.Id == newId) && newId > minId) {
    ...
    private void SetObjectTransparency(Appearance appearance, byte baseAlpha)
    {
        ...
        if (add)
        {
            Dictionary<MainWindow.Catalog, List<uint>> catalogSpriteIdMap = [];
    ...
    {
        Dictionary<MainWindow.Catalog, Dictionary<uint, byte>> catalogModifications = [];

        foreach (var modification in transparentSheets)
        {
            MainWindow.Catalog catalog = modification.Catalog;

            if (!catalogModifications.TryGetValue(catalog, out Dictionary<uint, byte>? value))
            {
                value = [];
                catalogModifications[catalog] = value;
            }

            foreach (uint spriteId in modification.SpriteIds)
            {
                value[spriteId] = modification.AlphaValue;
            }
        }
    ...
    private static void ChangeAlphaInRegion(byte[] pixels, int stride, int bytesPerPixel, System.Drawing.Rectangle region, byte newAlpha)
    {
    ...
    private static System.Drawing.Bitmap ConvertToFormat32bppArgb(System.Drawing.Bitmap bmp)
    {
