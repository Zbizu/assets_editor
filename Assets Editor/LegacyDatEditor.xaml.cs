...
/// </summary>
public partial class LegacyDatEditor : DatEditorCommon
{
    ...
    public override void UpdateShowList(int selection, uint? _ = null)
    {
        if (ObjListView != null)
        {
    ...
    protected override void ChangeGroupType(int group)
    {
        ...
        if (A_SprAnimation.Value > 1)
        {
            ...
            SprSynchronized.IsChecked = animation.AnimationMode == ANIMATION_ANIMATION_MODE.AnimationSynchronized ? true : false;
    ...
    protected override void InternalUpdateThingPreview()
    {
        try {
            ...
            bool isOutfit = CurrentObjectAppearance.AppearanceType == APPEARANCE_TYPE.AppearanceOutfit;
                ...
                if ((bool)SprBlendLayers.IsChecked == false) {
                    ...
                    for (int h = (int)spriteInfo.PatternHeight - 1; h >= 0; h--) {
                        for (int w = (int)spriteInfo.PatternWidth - 1; w >= 0; w--) {
                            int index = LegacyAppearance.GetSpriteIndex(frameGroup, w, h, layer, (int)Math.Min(direction, spriteInfo.PatternX - 1), addon, mount, (int)SprFramesSlider.Value);
                            int spriteId = (int)spriteInfo.SpriteId[index];
                            SetImageInGrid(SpriteViewerGrid, gridWidth, gridHeight, Utils.BitmapToBitmapImage(MainWindow.MainSprStorage.getSpriteStream((uint)spriteId)), counter, spriteId, index);
                            counter++;
                        }
                    }
                } else {
                    ...
                    for (int h = (int)spriteInfo.PatternHeight - 1; h >= 0; h--) {
                        for (int w = (int)spriteInfo.PatternWidth - 1; w >= 0; w--) {
                            int baseIndex = LegacyAppearance.GetSpriteIndex(frameGroup, w, h, 0, (int)Math.Min(direction, spriteInfo.PatternX - 1), 0, 0, (int)SprFramesSlider.Value);
                            ...
                            if (spriteInfo.PatternLayers > 1) {
                                int baseLayerIndex = LegacyAppearance.GetSpriteIndex(frameGroup, w, h, 1, (int)Math.Min(direction, spriteInfo.PatternX - 1), 0, 0, (int)SprFramesSlider.Value);
                            ...
                            using (System.Drawing.Graphics g = System.Drawing.Graphics.FromImage(baseBitmap)) {
                                ...
                                if ((bool)SprFullAddons.IsChecked) {
                                    for (int x = 1; x <= (int)SprAddonSlider.Maximum; x++) {
                                        int addonIndex = LegacyAppearance.GetSpriteIndex(frameGroup, w, h, 0, (int)Math.Min(direction, spriteInfo.PatternX - 1), x, 0, (int)SprFramesSlider.Value);
                                        ...
                                        int addonLayerIndex = LegacyAppearance.GetSpriteIndex(frameGroup, w, h, 1, (int)Math.Min(direction, spriteInfo.PatternX - 1), x, 0, (int)SprFramesSlider.Value);
                            ...
                            SetImageInGrid(SpriteViewerGrid, gridWidth, gridHeight, Utils.BitmapToBitmapImage(memoryStream), counter, 0, 0);
                            counter++;
                        }
                    }
                }
                    ...
                    for (int pw = 0; pw < spriteInfo.PatternX; pw++) {
                        for (int h = (int)(spriteInfo.PatternHeight - 1); h >= 0; h--) {
                            for (int w = (int)(spriteInfo.PatternWidth - 1); w >= 0; w--) {
                                ...
                                int spriteId = (int)spriteInfo.SpriteId[index];
                                SetImageInGrid(SpriteViewerGrid, gridWidth, gridHeight, Utils.BitmapToBitmapImage(MainWindow.MainSprStorage.getSpriteStream((uint)spriteId)), tileid, spriteId, index);
                                counter++;
                            }
                        }
                    }
    ...
    private void A_Texture_ValueChanged(object sender, RoutedPropertyChangedEventArgs<object> e)
    {
        ...
        if (frameworkElement.Name == "A_SprGroups")
        {
            if (A_SprGroups.Value == 1 && appearance.FrameGroup.Count == 2)
            {
                ...
                appearance.FrameGroup[0].FixedFrameGroup = FIXED_FRAME_GROUP.OutfitMoving;
            }
            else if (A_SprGroups.Value == 2 && appearance.FrameGroup.Count == 1)
            {
                ...
                appearance.FrameGroup[0].FixedFrameGroup = FIXED_FRAME_GROUP.OutfitIdle;
            }
        }
    ...
    private void ObjectSave_PreviewMouseLeftButtonDown(object sender, System.Windows.Input.MouseButtonEventArgs e)
    {
        ...
        list[ObjListView.SelectedIndex] = appearance.Clone();
    ...
    private static void WritePresetToOtfi(string otfiPath, PresetSettings preset, string datFile, string sprFile, bool transparency) {
        // create DatSpr node
            ...
            if (saveFileDialog.ShowDialog() == true)
            {
                foreach (var item in selectedItems)
                {
                    if (item.Image != null)
                    {
                        ...
                        {
                            case 1:
                                targetImg.Save(directoryPath + "\\" + item.Id.ToString() + ".bmp", ImageFormat.Bmp);
                                break;
                            case 2:
                                targetImg.Save(directoryPath + "\\" + item.Id.ToString() + ".gif", ImageFormat.Gif);
                                break;
                            case 3:
                                targetImg.Save(directoryPath + "\\" + item.Id.ToString() + ".jpg", ImageFormat.Jpeg);
                                break;
                            case 4:
                                targetImg.Save(directoryPath + "\\" + item.Id.ToString() + ".png", ImageFormat.Png);
                                break;
                        }
                        targetImg.Dispose();
                    }
            ...
            foreach (string filePath in selectedFiles)
            {
                using (System.Drawing.Image originalImage = System.Drawing.Image.FromFile(filePath))
                {
                    ...
                    for (int i = 0; i < cropsY; i++)
                    {
                        ...
                        {
                            System.Drawing.Rectangle cropRect = new(j * 32, i * 32, 32, 32);
                            using (System.Drawing.Bitmap croppedBitmap = new(32, 32, System.Drawing.Imaging.PixelFormat.Format32bppArgb))
                            {
    ...
    private void DeleteSprite_PreviewMouseLeftButtonDown(object sender, MouseButtonEventArgs e)
    {
        ...
        if (data != null && data.Image != null)
        {
            if(data.Id < MainWindow.SprLists.Count - 1)
            {
                ...
                using System.Drawing.Bitmap emptyBitmap = new(32, 32, System.Drawing.Imaging.PixelFormat.Format32bppArgb);
    ...
    {
        OTBEditor oTBEditor = new(this, true);
        oTBEditor.Show();
    }

    private void Import_Click(object sender, RoutedEventArgs e)
    {
        ImportManager importerManager = new(this);
        importerManager.Show();
    }

    private void SearchItem_Click(object sender, RoutedEventArgs e)
    {
        SearchWindow searchWindow = new(this, true);
        searchWindow.Show();
    }
    ...
    private void DeleteObject_PreviewMouseLeftButtonDown(object sender, MouseButtonEventArgs e)
    {
        ...
        if (ObjListView.SelectedIndex == ObjListView.Items.Count - 1) {
            list.RemoveAt(ObjListView.SelectedIndex);
            things.Remove((ShowList)ObjListView.SelectedItem);
        } else {
            Appearance selectedObject = new();
            selectedObject = list[ObjListView.SelectedIndex];
            selectedObject.Flags = new();
    ...
    private void NewObject_PreviewMouseLeftButtonDown(object sender, MouseButtonEventArgs e)
    {
        ...
        newObject.Id = (uint)(list.Count + 1);
        list.Add(newObject);
        ...
        if (e.Data.GetDataPresent(typeof(ShowList)))
        {
            ...
            if (result != null)
            {
                ...
                if (listViewItem != null)
                {
                    ...
                    var list = GetAppearanceList();
                    if (list != null) {
                        ReplaceObjectAppearance = list.FirstOrDefault(o => o.Id == data.Id);
                    }
                }
            }
